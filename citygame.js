...
case 'KeyC':
    ...
    raycaster = new THREE.Raycaster(new THREE.Vector3(), new THREE.Vector3(0, -2, 0), 0, 2)

    
    const light = new THREE.AmbientLight(0xffffff, 20);
    scene.add(light);

loader.load("./philipscity.glb", function(gltf){
    let city = gltf.scene;
    scene.add(city)
...
function animate() {
    ...
    if (isSprinting){
        camera.fov = 80
        camera.updateProjectionMatrix()
        if (moveForward || moveBackward) velocity.z -= directions.z * 1600.0 * delta
        if (moveLeft || moveRight) velocity.x -= directions.x * 1600.0 * delta
    
    }
    else if(isCrouching){
        camera.fov = 70
        camera.updateProjectionMatrix()
        if (moveForward || moveBackward) velocity.z -= directions.z * 200.0 * delta
        if (moveLeft || moveRight) velocity.x -= directions.x * 200.0 * delta
    }
else{
    camera.fov = 75
        camera.updateProjectionMatrix()
